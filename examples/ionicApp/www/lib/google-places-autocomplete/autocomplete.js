 ...
 *
 * MODIFIED TO BE USED WITH VIEWPOINT 2
 */
                            ...
                            $scope.$apply(function () {
                              ...
                              clearPredictions();
                                ...
                                if (status == google.maps.places.PlacesServiceStatus.OK) {
                                    ...
                                    if ($scope.options.stateFilter.length > 0) {
                                        var filteredPredictions = [];

                                        _.each(predictions, function(i) {
                                            _.each($scope.options.stateFilter, function(state) {
                                                if (i.description.indexOf(state) > -1) {
                                                    filteredPredictions.push(i)
                                                };
                                            });
                                        });

                                        $scope.predictions.push.apply($scope.predictions, filteredPredictions);
                                    } else {
                                ...
                                if ($scope.predictions.length > 7) {
                                    $scope.predictions.length = 7;  // trim predictions down to size
                                }
                            });
                        });
